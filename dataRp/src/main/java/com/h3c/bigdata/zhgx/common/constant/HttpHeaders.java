...
public final class HttpHeaders {
    ...
    public enum ContentType {
        TEXT_HTML(CONTENT_TYPE_TEXT_HTML), FORM(CONTENT_TYPE_FORM), JSON(CONTENT_TYPE_JSON), XML(CONTENT_TYPE_TEXT_HTML);

        private String val;

        ContentType(final String val) {
            this.val = val;
        }
    ...
    // -------------------以上内容来自 org.apache.http.HttpHeaders-----------------------------------------------------//

    /**
     * RFC.2183 (HTTP/1.1) Section.1.1 文件下载头
     */
    ...
    public static class UserAgent {
        ...
        private static final String DNET = ".NET";

        public static boolean isIE(String ua) {
            // .NET -> IE11
            return ua.indexOf(MSIE) > 0 || ua.indexOf(DNET) > 0;
        }
