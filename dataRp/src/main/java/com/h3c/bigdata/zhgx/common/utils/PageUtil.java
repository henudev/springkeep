...
/**
 * 分页工具类，用于处理不用使用pageHelper插件的列表分页，如数据服务/数据目录
 *
 * @param
 * @return
*/
public class PageUtil<T> {
    ...
    public PageUtil(List<T> instanceList, int size) {
        this.size = size;
    ...
    private List<T> currentPageData() {
        ...
        instanceList.stream().skip((this.current_page - 1) * this.size)
                .limit(this.size).forEach(e -> currentPageData.add(e));
